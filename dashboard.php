    ...
    <div class="col-3" style="width:20%;">
        <div class="card">
            ...
            <ul class="list-group list-group-flush">
                ...
                <li class="list-group-item"><a href="dashboard.php?mail">E-mail aanpassen</a></li>
                <li class="list-group-item"><a href="dashboard.php?password">Wachtwoord aanpassen</a></li>
    ...
    <div class="col-9" style="width:65%;">
        <div class="card">
                    ...
                    if (isset($_GET['name'])) {
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            if (trim($_POST['voornaam']) == '' && trim($_POST['achternaam']) == '') {
                                ...
                                echo '<div class="alert alert-danger" role="alert">Vul jouw voornaam in.</div>';
                            } elseif (trim($_POST['achternaam']) == '') {
                                echo '<div class="alert alert-danger" role="alert">Vul jouw achternaam in.</div>';
                            } else {
                                ...
                                if ($conn->query($qUpdateName) === TRUE) { //Voer query uit
                                    echo '<div class="alert alert-success">Je naam is aangepast.</div>';
                                } else {
                                    echo '<div class="alert alert-danger">Er is een fout opgetreden tijdens het aanpassen van jouw naam. Probeer het later nog eens.';
                                }
                            }
                        }
                    ?>
                <form action="settings.php?name" method="POST">
                    <label for="firstname" class="form-label">Voornaam:</label>
                    <input type="text" name="firstname" class="form-control" id="firstname" placeholder="Voornaam" value="<?php echo $fgebruikers['voornaam']; ?>">
                    <label for="lastname" class="form-label">Achternaam:</label>
                    <input type="text" name="lastname" class="form-control" id="lastname" placeholder="Achternaam" value="<?php echo $fgebruikers['achternaam']; ?>"><br />
            ...
            <?php
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            if (trim($_POST['email']) == '') {
                                echo '<div class="alert alert-danger" role="alert">Vul een email adres in.</div>';
                            } else {
                                $rEmail = $conn->real_escape_string($_POST['mail']);
                                ...
                                if ($conn->query($qUpdateMail) === TRUE) { //Voer query uit
                                    echo '<div class="alert alert-success">Je email is aangepast.</div>';
                                } else {
                                    echo '<div class="alert alert-danger">Er is een fout opgetreden tijdens het aanpassen van jouw email. Probeer het later nog eens.';
                                }
                            }
                        }
            ?>
                <form action="settings.php?mail" method="POST">
                    <label for="mail" class="form-label">Email:</label>
                    <input type="text" name="mail" class="form-control" id="mail" placeholder="Email" value="<?php echo $fgebruikers['email']; ?>"><br />
                    <button type="submit" class="btn btn-primary mb-3">Opslaan</button>
                </form>
            <?php
                    } elseif (isset($_GET['wachtwoord'])) {
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            if (trim($_POST['wachtwoord']) == '' && trim($_POST['wachtwoord1']) == '' && trim($_POST['wachtwoord2']) == '') {
                                ...
                                echo '<div class="alert alert-danger" role="alert">Vul jouw oude wachtwoord in.</div>';
                            } elseif (trim($_POST['wachtwoord1']) == '') {
                                echo '<div class="alert alert-danger" role="alert">Vul een nieuw wachtwoord in.</div>';
                                ...
                                echo '<div class="alert alert-danger" role="alert">De wachtwoorden komen niet overeen.</div>';
                            } else {
                                if (password_verify($_POST['wachtwoord'], $fUser['wachtwoord'])) {
                                    ...
                                    if ($conn->query($qUpdatePass) === TRUE) {
                                        echo '<div class="alert alert-success">Je wachtwoord is aangepast.</div>';
                                    } else {
                                        echo '<div class="alert alert-danger">Er is een fout opgetreden tijdens het aanpassen van jouw wachtwoord. Probeer het later nog eens.</div>';
                                    }
                                } else {
                                    echo '<div class="alert alert-danger">Je oude wachtwoord klopt niet.</div>';
                                }
                            }
                        }
            ?>
                ...
                <?php
                        ...
                        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                            if (trim($_POST['wachtwoord']) == '') {
                                echo '<div class="alert alert-danger">Vul jouw wachtwoord in.</div>';
                            } else {
                                if (password_verify($_POST['wachtwoord'], $fgebruikers['wachtwoord'])) { //Controleer of het ingevoerde wachtwoord juist is door het ingevulde wachtwoord te vergelijken met wat in de database staat
                                    ...
                                    if ($conn->query($qDelete) === TRUE) { //Voer de query uit
                                        echo '<div class="alert alert-success">Jouw account is verwijderd. Je wordt doorgestuurd naar de home pagina.</div>';
                ...
                <?php
                                    ...
                                    echo '<div class="alert alert-danger">Het wachtwoord klopt niet</div>';
                                }
                            }
                        }
                        echo 'Om jouw account te verwijderen moet je jouw wachtwoord opgeven.';
                ?>

                <form action="settings.php?delete" method="POST">
